...
public class LocalizationComponentAsset<T> where T : Object
{
    ...
    public async Task<T> GetAssetAsync(LocalizationField field)
    {
        if (localizedAssets.TryGetValue(field, out var localizedAsset))
        {
            ...
            Debug.LogWarning($"[ComponentLocalizationAsset] Failed to load localized asset for field {field}");
            return null;
        }

        Debug.LogWarning($"[ComponentLocalizationAsset] Missing localized asset for field {field}");
        return null;
    }
}
...
Usage:
[SerializeField]
private LocalizationComponentAsset<Sprite> portraitLocalization;

/// Initialize
portraitLocalization = new LocalizationComponentAsset<Sprite>(
